...
fn load_rsvo_rec<const BLOCK_SIZE: usvo, const LEVEL_COUNT: usize>(
    ...
    if level == level_count {
        set_from_rsvo(mem, level, position);
        return;
    }
    ...
    if children_mask == 0 {
        ...
        for i in 0..8 {
            ...
            if bit != 0 {
                load_rsvo_rec(rsvo, level_count, mem, indexes, level + 1, position + op * (2u32.pow((level_count - (level as usize) - 1) as u32)) as usvo);
            }
        }
    }
}

pub fn load_svo_from_rsvo<const BLOCK_SIZE: usvo, const LEVEL_COUNT: usize>(rsvo: &[u8]) -> SvoMut<BLOCK_SIZE, LEVEL_COUNT> {
    if BLOCK_SIZE != 2 && BLOCK_SIZE != 4 {
        panic!("BLOCK_SIZE must be 2 or 4");
    }
    ...
    let level_max = if BLOCK_SIZE == 2 { LEVEL_COUNT } else { LEVEL_COUNT * 2 };
    let level_count = rsvo32[4] as usize;
    if level_count < level_max {
        panic!("loading course into fine");
    }
    ...
    load_rsvo_rec(rsvo, level_max, &mut svo, &mut indexes, 0, Usvo3::new(0, 0, 0));
    svo
}
