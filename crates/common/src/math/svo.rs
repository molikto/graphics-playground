...
const MASK_IS_LEAF: usvo = 1u32 << (usvo::BITS - 1);
const MASK_DATA: usvo = !MASK_IS_LEAF;
...
pub struct BlockInfo<const LEVEL_COUNT: usize> {
    pub level: usvo,
    pub data: usvo,
}

impl<const LEVEL_COUNT: usize> BlockInfo<LEVEL_COUNT> {
...
#[repr(transparent)]
pub struct Svo<REF: Deref<Target = [usvo]>, const LEVEL_COUNT: usize> {
    pub mem: REF,
}

impl<REF: Deref<Target = [usvo]>, const LEVEL_COUNT: usize>
    Svo<REF, LEVEL_COUNT>
{
    pub const BLOCK_SIZE: usvo = BLOCK_DIM * BLOCK_DIM * BLOCK_DIM;
    pub const TOTAL_DIM: usvo = BLOCK_DIM.pow(LEVEL_COUNT as usvo);
    ...
    #[inline]
    pub fn new_block(terminal: bool, index: usvo) -> usvo {
        if terminal {
            return index;
        } else {
